...
class Caso2CalificacionesConLibro(TemplateView):
    ...
    def calificaciones_con_libro():
        califs = LibroCalificacion.objects.select_related(
            'libro'
        ).only(
            'estrellas', 
            'libro__titulo'
...
class Caso3AutoresConLibros(TemplateView):
    ...
    def autores_con_libros():
        qs = Libro.objects.all().only('titulo')
        prefetch = Prefetch('book', queryset=qs, to_attr='books')
        autores = Autor.objects.prefetch_related(prefetch)
        for a in autores:
            print(a.name, [libro.titulo for libro in a.books])
